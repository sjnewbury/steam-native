...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-1.0.8-makefile-dll.patch
)
...
multilib_src_compile() {
	if [[ ${CHOST} == *mingw* ]] ; then
		bemake -f "${S}"/Makefile-libbz2_dll LDFLAGS="${LDFLAGS} -Wl,--add-stdcall-alias" all
		# Link against the DLL implib 
		ln -s libbz2.dll.a libbz2.a || die
		bemake -f "${S}"/Makefile all LDFLAGS="${LDFLAGS} $(usex static -static '')"
		# Now rebuild the static library with kill-at
		rm -f libbz2.a *.o || die
		bemake -f "${S}"/Makefile LDFLAGS="${LDFLAGS} -static -Wl,--kill-at" libbz2.a
	else
...
multilib_src_install() {
	...
	if [[ ! -L "${ED}/usr/$(get_libdir)/libbz2.so.1.0" ]] && [[ ${CHOST} != *mingw* ]]; then
...
multilib_src_install_all() {
	...
	if [[ ${CHOST} == *mingw* ]] ; then
		dosym bzip2.exe /bin/bzcat
		dosym bzip2.exe /bin/bunzip2
	else
